...
def CX_loss_helper(vgg_A, vgg_B, CX_config):
    ...
    if fH * fW <= CX_config.max_sampling_1d_size ** 2:
        ...
        #vgg_A, vgg_B = random_pooling([vgg_A.detach().cpu(), vgg_B.detach().cpu()], output_1d_size=CX_config.max_sampling_1d_size)

    CX_loss = CSFlow.CX_loss(vgg_A, vgg_B, distance=CX_config.Dist, nnsigma=CX_config.nn_stretch_sigma)
    init = tf.global_variables_initializer()
    sess = tf.Session()
    sess.run(init)
    CX_loss_np = CX_loss.eval(session=sess)
    return CX_loss_np
